...
/* ────────────────────────────── 상세 패널: 한 줄 평 + 구매 정보 ────────────────────────────── */
function DetailPanel({ item, focusComment, suppliers, onEval, onUpdate, onRemove, onClose }) {
  const [tags, setTags] = useState(item.ev_tags || []);
  const [comment, setComment] = useState(item.ev_comment || '');
  const pick = it => ({ qty: Number(it.qty), unit: it.unit, unit_price: Number(it.unit_price), supplier: it.supplier, category: it.category, spec: it.spec, brand: it.brand, date: it.date, note: it.note });
  ...
  const saveBuy = () => {
    ...
    setErr(''); onUpdate(item.id, { ...f, supplier: f.supplier.trim(), spec: f.spec.trim(), note: f.note.trim() });
  };
  ...
  return (
    <div className="space-y-4 border-t border-slate-200 bg-slate-50 p-4">
      ...
      <div className="border-t border-slate-200 pt-4">
        <p className="mb-2 text-xs font-bold text-slate-600">구매 정보</p>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          <label className={lab}>수량<input id={`e-qty-${item.id}`} type="number" step="any" inputMode="decimal" value={f.qty} onChange={e => setF({ ...f, qty: +e.target.value })} className={`${input} mt-1`} /></label>
          <label className={lab}>단위<select id={`e-unit-${item.id}`} value={f.unit} onChange={e => setF({ ...f, unit: e.target.value })} className={`${input} mt-1`}>{UNITS.map(u => <option key={u}>{u}</option>)}</select></label>
          <label className={lab}>단가 (원)<input id={`e-price-${item.id}`} type="number" inputMode="numeric" value={f.unit_price} onChange={e => setF({ ...f, unit_price: +e.target.value })} className={`${input} mt-1`} /></label>
          <label className={lab}>입고일<input id={`e-date-${item.id}`} type="date" value={f.date} onChange={e => setF({ ...f, date: e.target.value })} className={`${input} mt-1`} /></label>
          <label className={lab}>거래처<input id={`e-sup-${item.id}`} list="dl-suppliers" value={f.supplier} onChange={e => setF({ ...f, supplier: e.target.value })} className={`${input} mt-1`} /></label>
          <label className={lab}>분류<select id={`e-cat-${item.id}`} value={f.category} onChange={e => setF({ ...f, category: e.target.value })} className={`${input} mt-1`}>{CATS.map(c => <option key={c}>{c}</option>)}</select></label>
          <label className={lab}>브랜드<select id={`e-brand-${item.id}`} value={f.brand} onChange={e => setF({ ...f, brand: e.target.value })} className={`${input} mt-1`}><option value="">선택 안 함</option>{BRANDS.map(b => <option key={b}>{b}</option>)}</select></label>
          <label className={lab}>규격·원산지<input id={`e-spec-${item.id}`} value={f.spec} onChange={e => setF({ ...f, spec: e.target.value })} className={`${input} mt-1`} /></label>
          <label className={`${lab} col-span-2 md:col-span-4`}>메모<input id={`e-note-${item.id}`} value={f.note} placeholder="LOT, 샘플 여부 등" onChange={e => setF({ ...f, note: e.target.value })} className={`${input} mt-1`} /></label>
        </div>
        {err && <p className="mt-2 flex items-center gap-1 text-sm font-semibold text-rose-700"><AlertTriangle size={14} />{err}</p>}
        ...
        <p className="mt-2 text-xs text-slate-400">등록 {item.requester || '—'} · {fmtAt(item.created_at)}</p>
      </div>
    </div>
  );
}
...
/* ────────────────────────────── 품목 행 (memo) ────────────────────────────── */
const PartRow = memo(function PartRow({ item, expanded, onVerdict, onEval, onToggleExpand, onUpdate, onRemove }) {
  const d = derive(item);
    ...
    <article className={`rounded-2xl border bg-white transition ${item.ev_v === 'unusable' ? 'border-rose-300' : 'border-slate-200'} ${item._syncing ? 'opacity-60' : ''}`}>
      <div className="grid gap-3 p-4 lg:grid-cols-12 lg:items-center">
        <div className="min-w-0 lg:col-span-4">
          ...
          <p className="truncate text-xs text-slate-500">{[item.supplier, item.spec].filter(Boolean).join(' · ') || '거래처 미입력'}</p>
        </div>
        <div className="flex items-baseline justify-between gap-3 lg:col-span-2 lg:block">
      ...
      {expanded && <DetailPanel item={item} focusComment={expanded === 'comment'} onEval={onEval} onUpdate={onUpdate} onRemove={onRemove} onClose={() => onToggleExpand(item.id, false)} />}
    </article>
  );
});

/* ────────────────────────────── 빠른 추가 (필수: 품목명·수량) ────────────────────────────── */
const QuickAdd = memo(function QuickAdd({ items, onAdd, onMerge }) {
  ...
  const blank = { name: '', qty: 1, unit: 'kg', unit_price: 0, supplier: '', category: '기타', brand: '', spec: '', note: '', date: todayStr() };
  const [f, setF] = useState(blank);
  const [more, setMore] = useState(false);
  const [err, setErr] = useState('');
  const set = patch => setF(s => ({ ...s, ...patch }));

  // 전에 산 품목이면 거래처·단가·분류·단위를 마지막 구매값으로 채움
  const fillFrom = name => {
    const p = latestByName.get(name);
    if (p) set({ name, unit: p.unit, unit_price: Number(p.unit_price), supplier: p.supplier, category: p.category, brand: p.brand, spec: p.spec });
    else set({ name });
  };
  ...
  const submit = e => {
    ...
    if (!f.date) return setErr('입고일을 선택하세요.');
    setErr('');
    onAdd({ ...f, name, supplier: f.supplier.trim(), spec: f.spec.trim(), note: f.note.trim() });
    set({ name: '', qty: 1, note: '' });
  };
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border-2 border-dashed border-orange-300 bg-orange-50 p-4">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1 text-sm font-bold text-orange-900"><Plus size={16} />구매 내역 추가</span>
        <span className="text-xs text-orange-800">품목명·수량만 넣고 Enter. 전에 산 품목은 거래처·단가가 자동으로 채워집니다</span>
      </div>
      {recent.length > 0 && (
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-12">
        <label className={`${lab} col-span-2 md:col-span-4`}>품목명
          <input id="qa-name" list="dl-names" value={f.name} maxLength={60} placeholder="예: 냉동 대왕쭈꾸미" onChange={e => set({ name: e.target.value })} onBlur={e => latestByName.has(e.target.value) && fillFrom(e.target.value)} className={`${field} mt-1`} />
        </label>
        <div className={`${lab} col-span-2 sm:col-span-1 md:col-span-3`}>수량
          <div className="mt-1 flex h-12 overflow-hidden rounded-xl border border-slate-300 bg-white">
            <button type="button" aria-label="수량 감소" onClick={() => set({ qty: Math.max(step, +(f.qty - step).toFixed(2)) })} className="w-11 shrink-0 text-slate-600 hover:bg-slate-100"><Minus size={18} className="mx-auto" /></button>
            <input id="qa-qty" type="number" step="any" inputMode="decimal" value={f.qty} onChange={e => set({ qty: +e.target.value })} className="min-w-0 flex-1 text-center text-base font-bold tabular-nums focus:outline-none" />
            <button type="button" aria-label="수량 증가" onClick={() => set({ qty: +(f.qty + step).toFixed(2) })} className="w-11 shrink-0 text-slate-600 hover:bg-slate-100"><Plus size={18} className="mx-auto" /></button>
            <select id="qa-unit" aria-label="단위" value={f.unit} onChange={e => set({ unit: e.target.value })} className="shrink-0 border-l border-slate-300 bg-slate-50 px-2 text-sm font-semibold focus:outline-none">{UNITS.map(u => <option key={u}>{u}</option>)}</select>
          </div>
        </div>
        <label className={`${lab} md:col-span-2`}>단가 (원/{f.unit})
          <input id="qa-price" type="number" inputMode="numeric" value={f.unit_price} onChange={e => set({ unit_price: +e.target.value })} className={`${field} mt-1 tabular-nums`} />
        </label>
        <label className={`${lab} md:col-span-3`}>거래처
          <input id="qa-sup" list="dl-suppliers" value={f.supplier} placeholder="예: 해심트레이딩" onChange={e => set({ supplier: e.target.value })} className={`${field} mt-1`} />
        </label>
        <label className={`${lab} col-span-1 md:col-span-3`}>분류
          <select id="qa-cat" value={f.category} onChange={e => set({ category: e.target.value })} className={`${field} mt-1`}>{CATS.map(c => <option key={c}>{c}</option>)}</select>
        </label>
        <label className={`${lab} col-span-1 md:col-span-3`}>브랜드
          <select id="qa-brand" value={f.brand} onChange={e => set({ brand: e.target.value })} className={`${field} mt-1`}><option value="">선택 안 함</option>{BRANDS.map(b => <option key={b}>{b}</option>)}</select>
        </label>
        <div className="col-span-2 flex items-end md:col-span-6">
          <button type="submit" className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-orange-600 text-base font-bold text-white hover:bg-orange-700">
            <Plus size={18} />추가 · {fmtKRW((f.qty || 0) * (f.unit_price || 0))}
          </button>
        </div>
      </div>
      <button type="button" onClick={() => setMore(m => !m)} className="mt-2 text-xs font-semibold text-orange-800 underline">{more ? '옵션 닫기' : '입고일·규격·메모 입력'}</button>
      {more && (
        <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-3">
          <label className={lab}>입고일<input id="qa-date" type="date" value={f.date} onChange={e => set({ date: e.target.value })} className={`${field} mt-1`} /></label>
          <label className={lab}>규격·원산지<input id="qa-spec" value={f.spec} placeholder="예: 200g×10 · 국내산" onChange={e => set({ spec: e.target.value })} className={`${field} mt-1`} /></label>
          <label className={`${lab} col-span-2 md:col-span-1`}>메모<input id="qa-note" value={f.note} placeholder="LOT, 샘플 여부 등" onChange={e => set({ note: e.target.value })} className={`${field} mt-1`} /></label>
        </div>
      )}
      {dup && (
        <div className="mt-3 flex flex-wrap items-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-900">
          <AlertTriangle size={16} />같은 날 같은 거래처의 <b>{dup.name}</b>({fmtQty(dup.qty)}{dup.unit})이 이미 있습니다.
          <button type="button" onClick={() => { onMerge(dup.id, f.qty); set({ name: '', qty: 1 }); }} className="ml-auto inline-flex h-10 items-center gap-1 rounded-lg bg-amber-600 px-3 text-sm font-bold text-white"><GitMerge size={14} />수량 합산</button>
        </div>
      )}
      {err && <p className="mt-2 flex items-center gap-1 text-sm font-semibold text-rose-700"><AlertTriangle size={14} />{err}</p>}
    </form>
...
function StatsView({ items, activity }) {
  const s = useMemo(() => {
    ...
    const byDate = {}; const cat = {}; const sup = {}; const reason = {};
    ...
    return { dow, daily, verdict, cat: toBars(cat), sup: toBars(sup).slice(0, 8), reason: Object.entries(reason).map(([name, 건수]) => ({ name, 건수 })).sort((a, b) => b.건수 - a.건수) };
  }, [items]);
...
function App() {
  ...
  useEffect(() => { setLimit(PAGE); }, [q, fVerdict, fCat, fBrand, period]);
  ...
  const clearFilters = () => { setQuery(''); setFVerdict([]); setFCat([]); setFBrand([]); };

  const renderRow = useCallback(it => (
    <PartRow key={it.id} item={it} expanded={expanded?.id === it.id ? expanded.mode : false}
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-4 sm:py-6">
        ...
        <section className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <input id="search" ref={searchRef} value={query} onChange={e => setQuery(e.target.value)} placeholder="품목·거래처·원산지·한 줄 평·작성자 검색  ( / )"
        ...
        {status === 'error' ? (
          ...
          <div className="space-y-3">
            <QuickAdd items={items} onAdd={addItem} onMerge={mergeQty} />
